...
COCO_WEIGHTS_PATH = os.path.join(ROOT_DIR, "custom_weights.h5")
...
class RecycleDataset(utils.Dataset):

    def load_recycle(self, dataset_dir, subset):
        """Load a subset of the Recycle dataset.
        ...
        for im_class in class_names:
            class_dataset_dir = os.path.join(dataset_dir, im_class)
            ...
            # print(im_class, subset)
            annotations = json.load(open(os.path.join(class_dataset_dir, "via_region_data.json")))
            ...
            for a in annotations:
                ...
                image_path = os.path.join(class_dataset_dir, a['filename'])
                ...
                self.add_image(
                    ...
                    polygons=polygons,
                    shapes=class_names.index(im_class)+1)
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        return mask.astype(np.bool), np.ones([mask.shape[-1]], dtype=np.int32) * image_info["shapes"]
...
def train(model, epochs = 30, style = 'heads'):
    """Train the model."""
    ## Training dataset.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--style', required=False,
                        metavar="type of transfer learning to use",
                        help='number of layers to train on (head or all)')
    ...
    if args.command == "train":
        ...
        assert args.image or args.video,\
               "Provide --image or --video to apply inference"
